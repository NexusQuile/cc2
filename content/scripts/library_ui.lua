...
function imgui_options_menu(ui, x, y, w, h, is_active, selected_category_index, is_highlight)
    ...
    elseif selected_category_index == 6 then
        ...
        ui:end_window()
	elseif selected_category_index == 7 then
		
		local back_col = color8(255, 255, 255, 5)
		local window = ui:begin_window("##preview", x, y, w, h/2 - 5, atlas_icons.column_profile, is_active, 0, true, is_highlight)
		local local_w, local_h = ui:get_region()
		...
		update_ui_push_offset(local_w / 2 - 64 - 10, local_h / 2 - 32)
		...
		update_ui_push_offset(local_w / 2 + 10, local_h / 2 - 32)
		update_ui_rectangle(-5, -5, 74, 74, back_col)
		...
		ui:begin_window(update_get_loc(e_loc.upp_profile), x, y+h/2, w, h/2, atlas_icons.column_profile, is_active)
		ui:header(update_get_loc(e_loc.upp_body))
        --print("Gender before combo = ",settings.character_gender)
		...
		--print("Gender after combo = ",settings.character_gender)
		...
		local red = settings.character_skin_color:r()
		local green = settings.character_skin_color:g()
		local blue = settings.character_skin_color:b()		
		
		ui:header("Red="..red)
		local val_change = { -10, -1, 1, 10}
        local inc_act = ui:button_group({ "-10", "-1", "+1", "+10" }, true)
        if inc_act >= 0 then
			red = math.max(0, math.min( 255, red + val_change[inc_act + 1]))
        end
		ui:header("Green="..green)
		local inc_act = ui:button_group({ "-10", "-1", "+1", "+10" }, true)
        if inc_act >= 0 then
			green = math.max(0, math.min( 255, green + val_change[inc_act + 1]))
        end
		ui:header("Blue="..blue)
		local inc_act = ui:button_group({ "-10", "-1", "+1", "+10" }, true)
        if inc_act >= 0 then
			blue = math.max(0, math.min( 255, blue + val_change[inc_act + 1]))
        end	
		settings.character_skin_color = color8(red, green, blue, 255)       
		...
		local red = settings.character_hair_color:r()
		local green = settings.character_hair_color:g()
		local blue = settings.character_hair_color:b()
		...
		settings.character_hair_color = color8(red, green, blue, 255)
		ui:end_window()

		local settings_prev = update_get_game_settings()
		
		--for k, v in pairs(settings) do
		--	print(k, v, settings[k], settings_prev[k])
		--	if v ~= settings_prev[k] and type(v) ~= "userdata" then
		--		update_ui_event("set_game_setting", k, v)
		--		print("Updated Setting", k, "from", settings_prev[k], "to", v)
		--	end
		--end
